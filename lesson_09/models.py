...
class ContactsMessage(DomainObject):
...
class Model:
    ...
    def create_message(email, subject, message):
        return ContactsMessage(email, subject, message)
...
class ContactsMapper:
    ...
    def all(self):
        statement = f"SELECT id, email, subject, message from {self.table};"
        self.cursor.execute(statement)
        result = []
        for item in self.cursor.fetchall():
            id, email, subject, message = item
            obj = ContactsMessage(email, subject, message)
            obj.email = email
            obj.subject = subject
            obj.message = message
            result.append(obj)
    ...
    def find_by_id(self, id):
        statement = f"SELECT id, email, subject, message from {self.table} WHERE id = ?;"
        self.cursor.execute(statement, (id,))
        result = self.cursor.fetchone()
        try:
            id, email, subject, message = result
            ContactsMessage(email, subject, message)
        except:
            raise NotFoundException(f'Id {id} not found')

    def insert(self, obj):
        statement = f"INSERT INTO {self.table} (email, subject, message) VALUES (?, ?, ?);"
        self.cursor.execute(statement, (obj.email, obj.subject, obj.message))
        try:
            self.connection.commit()
        except Exception as e:
            raise CommitException(e.args)
...
class MapperRegistry:
    ...
    def get_mapper(obj):
        ...
        elif isinstance(obj, ContactsMessage):
            return ContactsMapper(connection)
