...
class Contacts:
    def __call__(self, request):
        if request['method'] == 'POST':
            ...
            new_message = site.create_message(email, subject, message)
            site.messages.append(new_message)
            new_message.mark_new()
...
class Messages:
    def __call__(self, request):
        mapper = MapperRegistry.get_current_mapper('message')
        return '200 OK', render('messages.html', messages_list=mapper.all())
